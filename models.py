...
class BERT_BiLSTM_CRF(BertPreTrainedModel):
    ...
    def __init__(self, config, task_infos, need_sac, need_birnn, need_charcnn, need_cnn, cnn_out_dim, share_cnn,
                 char_vocab_size, char_embedding_dim, tag_num, sac_factor, tag_fearure_dim=25, char_out_dim=300,
                 rnn_dim=128, device="cpu"):
        ...
        self.birnn_dropout = nn.Dropout(0.5)
        self.cnn_dropout = nn.Dropout(0.5)
        out_dim = config.hidden_size

        if need_charcnn:
            if not share_cnn:
                self.char_cnns = nn.ModuleList([CharCNN(vocab_size=self.char_vocab_size, embed_dim=char_embedding_dim,
                                                        kernel_num=char_out_dim // 3)] * len(task_infos))
            else:
                self.char_cnn = CharCNN(vocab_size=self.char_vocab_size, embed_dim=char_embedding_dim,
                                        kernel_num=char_out_dim // 3)
            out_dim += char_out_dim

        if need_birnn:
            self.birnn = nn.ModuleList(
                [nn.LSTM(out_dim, rnn_dim, bidirectional=True, batch_first=True).to(device)] * len(task_infos))
            out_dim = rnn_dim * 2

        if self.need_cnn:
            self.cnns = nn.ModuleList([nn.Conv2d(1, cnn_out_dim // 3, (K, out_dim)) for K in [1, 3, 5]])
            out_dim = cnn_out_dim

        self.hidden2tag = nn.ModuleList(
            [nn.Linear(out_dim, self.num_tags[i]).to(device) for i in range(len(self.num_tags))])
        self.crf = nn.ModuleList(
            [CRF(self.num_tags[i], batch_first=True).to(device) for i in range(len(self.num_tags))])

        if need_sac:
            self.sac_factor = sac_factor
            self.sac = SAC(feature_dim=out_dim, tag_fearure_dim=tag_fearure_dim, tag_num=tag_num)

        # shape (batch, max_seq_len, feature_dim)
    ...
    def tag_outputs(self, task_id, input_ids, char_ids, input_mask):
        ...
        if self.need_charcnn:
            ...
            char_sequence_output = char_cnn_out.reshape(batch_size, -1, char_cnn_out.shape[
                -1])  # (batch_size, max_sent_len, char_out_dim)
        ...
        if self.need_birnn:
            ...
            sequence_output = self.birnn_dropout(sequence_output)

        # print("seq shape", sequence_output.shape)
        if self.need_cnn:
            # print(" cnn input", sequence_output.shape)
            sequence_output = sequence_output.unsqueeze(1)
            # print(" cnn input", sequence_output.shape)
            sequence_output = [
                F.relu(conv(F.pad(sequence_output, (0, 0, conv.kernel_size[0]//2, conv.kernel_size[0]//2)))).squeeze(3) for conv in
                self.cnns]
            # print(" cnn output", [x.shape for x in sequence_output])
            # sequence_output = [F.max_pool1d(i, i.size(2)).squeeze(2) for i in sequence_output]
            sequence_output = torch.cat(sequence_output, 1)
            # print("cat ", sequence_output.shape)
            sequence_output = sequence_output.permute(0, 2, 1)
            sequence_output = self.cnn_dropout(sequence_output)
            
            # print("permute ", sequence_output.shape)
